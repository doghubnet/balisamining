      ...
      <section id="projects" className="bg-navy px-6 py-20">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 flex snap-x gap-6 overflow-x-auto pb-4">
            {[
              '5-Star Hotel Lobby in Dubai',
              'Luxury Villa Collection in Milan',
              'Corporate Tower Atrium in New York',
              'Seaside Resort in Antalya'
            ].map((project, i) => (
              <motion.div key={project} whileHover={{ scale: 1.02 }} className="min-w-[300px] snap-start overflow-hidden rounded-2xl border border-white/10 bg-slate-900/70 md:min-w-[420px]">
                <img
                  src={`https://images.unsplash.com/photo-1617098474202-0d0d7f60f5d3?auto=format&fit=crop&w=1200&q=80&sig=${i + 1}`}
                  alt={project}
